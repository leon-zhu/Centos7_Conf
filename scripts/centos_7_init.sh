...
yum install -y net-tools.x86_64 vim-enhanced zlib-devel bzip2-devel openssl-devel ncurses-devel sqlite-devel \
                readline-devel tk-devel gcc make libffi-devel wget epel-release
...
# Java env
cd ${SOFTWARE_HOME} && wget 'https://xxx/jdk-8u261-linux-x64.tar.gz'
tar -zxvf jdk-8u251-linux-x64.tar.gz -C ${INSTALL_HOME}
